...
export class MenuItemsService {
    ...
    /* TODO: complete getMenuItems so that it returns a nested menu structure
      ...
      Hints:
      - open the `src/menu-items/menu-items.service.ts` f
    ...
    */
    ...
    getNestItem(items: MenuItem[], current: any): any {
        if (current == null) {
            ...
            return roots.map((r) => {
                ...
                this.getNestItem(items, tempRoot);
                return tempRoot;
            });
        } else {
            current.children = items.filter((ic) => ic.parentId == current.id);
            current.children.forEach((c: any) => {
                this.getNestItem(items, c)
            })
            return current;
        }
    }
    ...
    async getMenuItems() {
        const menuItem = await this.app.getDataSource().menuItem.findMany();
        const res = this.getNestItem(menuItem, null);
        return res;
    }
}
